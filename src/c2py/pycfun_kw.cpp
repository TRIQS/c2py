...
bool c2py::pycfun_kw::is_callable(PyObject *, PyObject *args, PyObject *kwargs, bool raise_exception) const noexcept {
  ...
  long n_args_py            = (args == nullptr ? 0 : PyTuple_Size(args));
  long n_kwargs_py          = (kwargs == nullptr ? 0 : PyDict_Size(kwargs));
  long n_args_tot_py        = n_args_py + n_kwargs_py;

  // Check the number of arguments
  if (n_args_tot_py > n_arg_max) {
    if (raise_exception) {
      PyErr_SetString(PyExc_TypeError,
                      ("Too many arguments. Expected at most "s + std::to_string(n_arg_max) + " and got " + std::to_string(n_args_tot_py)).c_str());
    }
    return false;
  }

  if (n_args_tot_py < n_arg_max - n_arg_with_default_c) {
    if (raise_exception) {
      PyErr_SetString(
         PyExc_TypeError,
         ("Too few arguments. Expected at least "s + std::to_string(n_arg_max - n_arg_with_default_c) + " and got " + std::to_string(n_args_tot_py))
            .c_str());
    }
    return false;
  }
  ...
  for (long pos = n_args_py; pos < n_arg_max; pos++) {
  ...
  if (n_picked_in_dict != n_kwargs_py) {
    if (raise_exception) {
...
PyObject *c2py::pycfun_kw::operator()(PyObject *self, PyObject *args, PyObject *kwargs) const {
  ...
  // FIXME : completely rework teh signal handling ...
  //auto lock__1 = signal_handler_lock{};
  try {
    if (not release_GIL) {
