...
namespace c2py {
  ...
  // and py_type<std::function<...>> is a wrapped callable object
  // The converter will :
  ...
  struct py_converter<std::function<R(T...)>> {
    ...
    using py_t = py_type<c_t>;

    static bool is_pytype_ready() {
      static bool ready = false;
      if (not ready) {
        if (PyType_Ready(&py_type_object<c_t>) < 0) return false;
        ready = true;
      }
      return true;
    }
    ...
    template <typename U> static PyObject *c2py(U &&x) {
      ...
      py_t *self = (py_t *)py_type_object<c_t>.tp_alloc(&py_type_object<c_t>, 0);
    ...
    static c_t py2c(PyObject *ob) {
      ...
      if (PyObject_TypeCheck(ob, &py_type_object<c_t>)) { return *(((py_t *)ob)->_c); }
