...
#include <c2py/converters/string.hpp>
#include <functional>

// --- call ---

int call1 (std::function<int(int,int)> const & f) { 
  return f(1,2);
}
...
int call3 (std::function<int(std::pair<int,int>)> const & f) { 
  return f(std::pair{1,2});
}

int call4 (std::function<int(std::pair<int,int>)> f) { 
  return f(std::pair{1,2});
}

int call5 (std::function<int(std::pair<int,int> const &)> f) { 
  return f(std::pair{1,2});
}
